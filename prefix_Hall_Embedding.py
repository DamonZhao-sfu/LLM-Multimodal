...
def run_experiment_with_cached_embeddings(
    ...
    result_df = spark.sql("""
        SELECT 
            gt_answer,
            LLM('Given the text: {text:question} and image: {image:image}, give me the answer to the question, return 1 as yes and 0 as no', question, image) as predicted
        FROM pope
    """)
    ...
    "is_correct",
    when(
        lower(col("predicted")).contains(lower(trim(col("gt_answer")))),
        1
    ).otherwise(0))
...
if __name__ == "__main__":
    keep_ratios = [1]
    dataset_name = "hall_image_prefix"
    
    overall_start = time.time()
    
    # Read POPE parquet once
    POPE_PATH = "/scratch/hpc-prf-haqc/haikai/dataset/hallusionBench/image-00000-of-00001.parquet"
