...
class EncoderCacheManager:
    ...
    def check_and_update_cache(self, request: Request, input_id: int) -> bool:
        ...
        if (self.debug_hit_rate_steps and 
            self.total_requests % self.debug_hit_rate_steps == 0):
    ...
    def _print_cache_stats(self) -> None:
        ...
        used_slots = self.cache_size - self.num_free_slots
        utilization = used_slots / self.cache_size if self.cache_size > 0 else 0
        
        print(f"EncoderCache Stats:")
        print(f"  Hit Rate: {hit_rate:.2%} ({self.cache_hits}/{self.total_requests})")
        print(f"  Cache Utilization: {utilization:.2%} ({used_slots}/{self.cache_size} tokens)")
        print(f"  Active Requests: {len(self.cached)}")
