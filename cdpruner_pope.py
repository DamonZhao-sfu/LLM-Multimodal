...
def execute_batch_pope_with_pruned_embeddings(
    ...
    vision_tower, model, _ = load_vision_models(device='cuda')
    try:
...
#     vision_tower, model = load_vision_models(device='cuda')
...
# def create_llm_udf_with_embeddings(keep_ratio: float):
#     """Create UDF that returns both prediction and pruning time"""
    
#     # Define schema for return value (prediction, pruning_time)
#     return_schema = StructType([
#         StructField("predicted", StringType(), True),
#         StructField("pruning_time", FloatType(), True)
#     ])
...
#         # Calculate per-row pruning time (total batch time divided by number of rows)
#         per_row_pruning_time = batch_pruning_time / len(outputs) if outputs else 0.0
        
#         # Return DataFrame with both predictions and pruning times
#         return pd.DataFrame({
#             'predicted': outputs,
#             'pruning_time': [per_row_pruning_time] * len(outputs)
#         })
...
#             answer,
#             image_source,
#             LLM('Given the text: {text:question} and image: {image:image} give me the answer to the question', question, image) as llm_result
#         FROM pope
#     """)
    
#     # Extract predicted and pruning_time from struct
...
#         col("answer"),
#         col("image_source"),
#         col("llm_result.predicted").alias("predicted"),
#         col("llm_result.pruning_time").alias("pruning_time")
#     )
...
#     # Calculate total pruning time by summing the pruning_time column
#     pruning_time_result = result_df_with_comparison.agg({"pruning_time": "sum"}).collect()
#     pruning_time = pruning_time_result[0][0] if pruning_time_result and pruning_time_result[0][0] else 0.0
    
#     # Drop question and pruning_time before saving
#     result_df_final = result_df_with_comparison.drop("question", "pruning_time")
    
#     # Write results to CSV
#     output_path = f"./{dataset_name}_{keep_ratio}.csv"
#     result_df_final.coalesce(1).write.mode("overwrite").option("header", "true").csv(output_path)
    
#     end_time = time.time()
#     execution_time = end_time - start_time
    
#     print(f"\nExecution time for keep_ratio={keep_ratio}: {execution_time:.2f} seconds")
#     print(f"Total pruning time: {pruning_time:.2f} seconds ({pruning_time/execution_time*100:.2f}% of total)")
