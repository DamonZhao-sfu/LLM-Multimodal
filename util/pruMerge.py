...
# Documentation and logging utilities
...
def outlier_dectection(attn):
    attn_np = attn.detach().to(dtype=torch.float32).cpu().numpy().flatten()
...
# ---------------------------------------------------------------------------
# NOTE: The following helper functions are assumed to be defined elsewhere
# ---------------------------------------------------------------------------
#
# def hook_k(module, input, output):
#     """Hook function to capture K values. Modifies a global 'outputs' dict."""
#     global outputs
#     outputs["desired_k"] = output
#
# def hook_q(module, input, output):
#     """Hook function to capture Q values. Modifies a global 'outputs' dict."""
#     global outputs
#     outputs["desired_q"] = output
#
# def outlier_dectection(cls_attn):
#     """Function to adaptively calculate the reduction ratio."""
#     # ... implementation needed ...
#     return 0.25 # Placeholder
#
# def complement_idx(idx, N):
#     """Function to get the complement indices."""
#     # ... implementation needed ...
#     B, _ = idx.shape
#     full_idx = torch.arange(N, device=idx.device).unsqueeze(0).expand(B, -1)
#     mask = torch.ones(B, N, dtype=torch.bool, device=idx.device)
#     mask.scatter_(1, idx, False)
#     return full_idx[mask].reshape(B, -1) # Placeholder implementation
#
# global outputs
# ---------------------------------------------------------------------------

# @torch.no_grad()
# def pruMerge(model, image_binary, processor, keep_ratio=0.25):
...
#     # === 2. Image Processing (mirroring forward pass logic) ===
#     encode_start = time.time()
    
#     # Calculate number of patches (from forward pass)
#     if image_sizes is not None:
#         image_num_patches = [
...
#     # Handle 5D pixel_values (multi-patch)
...
#             img_feature = model.token_prune_merge_advanced_plus(
#                 img.unsqueeze(0), 
#                 if_adaptive=True
#             )
#         else:
#             img_feature = model.token_prune_merge_advanced(
#                 img.unsqueeze(0), 
#                 if_adaptive=True
#             )
        
#         # Project to LLM space
#         img_feature = model.multi_modal_projector(img_feature)
#         image_features.append(img_feature)
    
#     # Concatenate all features
#     if len(image_features) > 1:
#         # Multiple patches - concatenate along token dimension
#         image_features = torch.cat(image_features, dim=1)  # [1, total_tokens, hidden_dim]
#     else:
#         # Single patch
#         image_features = image_features[0]  # [1, num_tokens, hidden_dim]
    
#     encode_time = time.time() - encode_start
#     prune_time = time.time() - prune_start
#     image_features = image_features.squeeze(0)
#     return image_features, preprocess_time, encode_time, prune_time
...
class LlavaNextForConditionalGeneration(LlavaNextPreTrainedModel):
    ...
    def token_prune_merge_advanced_plus(self, images, if_adaptive=True, reduction_ratio = 0.2):
        ...
        #set hooks for extracting desired layer's k and q
            ...
            if if_adaptive:
                ...
                new_idx_list = []
                for b in range(B):
                    batch_idx = idx[b]  # [num_selected]
                    
                    mask = torch.ones(arithmetic_sequence.shape[0], dtype=torch.bool, device=self.device)
                    for selected_idx in batch_idx:
                        mask = mask & (arithmetic_sequence != selected_idx)
                    
                    filtered_sequence = arithmetic_sequence[mask]
                    
                    # Concatenate
                    concatenated = torch.cat((batch_idx, filtered_sequence), dim=0)
                    new_idx_list.append(concatenated)
                
                # Find max length and pad
                max_len = max(t.shape[0] for t in new_idx_list)
                padded_idx = torch.zeros((B, max_len), dtype=torch.long, device=self.device)
                for b, t in enumerate(new_idx_list):
                    padded_idx[b, :t.shape[0]] = t
    ...
    def prumerge(self, inputs):
        # 1. Move inputs to correct device (from previous fix)
        device = self.get_input_embeddings().weight.device
        pixel_values = inputs.pixel_values.to(device)
        input_ids = inputs.input_ids.to(device)
        attention_mask = inputs.attention_mask.to(device)

        # --- FIX START: Collapse expanded image tokens ---
        # The processor expanded the image to 2144 tokens, but _merge expects 1 token.
        # We must reduce consecutive image tokens to a single token.
        
        # Check if we have more image tokens than actual images (assuming Batch Size = 1)
        num_image_tokens = (input_ids == self.config.image_token_index).sum()
        if num_image_tokens > 1:
            # Flatten for easier processing (assuming B=1)
            flat_ids = input_ids.squeeze(0)
            flat_mask = attention_mask.squeeze(0)
            
            is_image = (flat_ids == self.config.image_token_index)
            
            # Logic: Keep token if (It is NOT an image) OR (It IS an image but the PREVIOUS one was NOT)
            # This effectively keeps only the first token of any contiguous block of image tokens
            shifted_is_image = torch.cat([torch.tensor([False], device=device), is_image[:-1]])
            keep_indices = (~is_image) | (is_image & ~shifted_is_image)
            
            # Apply filter and restore batch dimension
            input_ids = flat_ids[keep_indices].unsqueeze(0)
            attention_mask = flat_mask[keep_indices].unsqueeze(0)
        # -------------------------------------------------
