...
def getCDPrunedVisualToken(model, vision_tower, image_binary, texts, keep_ratio=0.25):
    ...
    try:
        ...
        if 'images' in locals():
            del images
        if 'image_forward_outs' in locals():
            del image_forward_outs
        if 'image_outputs' in locals():
            del image_outputs
        ...
        if 'image_embeds' in locals():
            del image_embeds
        if 'mm_projector' in locals():
            del mm_projector
        if 'image_normalized' in locals():
            del image_normalized
        if 'similarity' in locals():
            del similarity
        if 'relevance' in locals():
            del relevance
        if 'kernel' in locals():
            del kernel
        if 'cis' in locals():
        ...
        if 'di2s' in locals():
            del di2s
        if 'select_idx' in locals():
            del select_idx
        if 'index_masks' in locals():
            del index_masks
