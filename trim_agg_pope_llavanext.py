...
def run_experiment(keep_ratio: float, dataset_name: str = "POPE_random") -> Tuple[str, float, float]:
    ...
    result_df_with_comparison = spark.sql("""
        SELECT 
            ...
            answer,
            image_source,
            LLM('Look at this image: {image:image}. return "Yes" if the image contains anything, otherwise "No"', image) as predicted
        FROM pope
    """)
    
    #result_df_with_comparison = spark.sql("""SELECT id, question, image_source FROM pope WHERE LLM('Look at this image: {image:image}. return "Yes" if the image contains anything, otherwise "No"', image) = 'Yes'""")
...
if __name__ == "__main__":
    ...
    dataset_name = "trim_agg_POPE_llavanext"
